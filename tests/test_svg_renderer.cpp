...
TEST_F(SvgRendererTest, AllWhiteCells_NoBlackRects) {
    BarcodeMatrix m(3, 3); // все false по умолчанию
...
TEST_F(SvgRendererTest, OneBlackCell_ExactlyOneBlackRect) {
    BarcodeMatrix m(3, 3);
    m.Set(0, 0, true);
    EXPECT_EQ(CountOccurrences(renderer.Render(m, defaults), "fill=\"black\""), 1u);
}
...
TEST_F(SvgRendererTest, MultipleBlackCells_CorrectCount) {
    ...
    m.Set(1, 1, true);
    m.Set(2, 2, true);
    EXPECT_EQ(CountOccurrences(renderer.Render(m, defaults), "fill=\"black\""), 3u);
}

// 8. Выход за границы матрицы бросает исключение
TEST_F(SvgRendererTest, OutOfBounds_ThrowsException) {
    BarcodeMatrix m(2, 2);
    EXPECT_THROW(m.Get(5, 5), std::out_of_range);
    EXPECT_THROW(m.Set(5, 5, true), std::out_of_range);
}